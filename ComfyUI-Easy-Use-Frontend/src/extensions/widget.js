...
import {createVueWidget} from "@/extensions/customWidget.js";
import loraStackWidget from "@/components/graph/widgets/loraStackWidget.vue";
...
const loraStackWidgetPattern = /^lora_\d+_(enabled|name|strength|model_strength|clip_strength)$/
const hideLoraStackWidgets = (node) => {
    node.widgets?.forEach(widget => {
        if (['toggle', 'mode', 'num_loras'].includes(widget.name) || loraStackWidgetPattern.test(widget.name)) {
            toggleWidget(node, widget, false)
        }
    })
}

const mountLoraStackWidget = (node) => {
    const hasWidget = node.widgets?.some(widget => widget.name === 'lora_stack_editor')
