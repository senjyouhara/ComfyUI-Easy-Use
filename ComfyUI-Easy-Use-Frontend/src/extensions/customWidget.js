...
export const createVueWidget = (obj, components) => {
  const container = document.createElement('div')
  const node = obj.node
  const options = obj.options || {getMinHeight: () => 100, hideOnZoom: true, serialize: true}
  const styles = obj.styles || null
  const widgetName = obj.name || 'ustom_vue_component_basic'
  const appKey = `easyuse-vue-widget-${node.id}-${widgetName.replace(/\s+/g, '-')}`
  container.id = appKey
